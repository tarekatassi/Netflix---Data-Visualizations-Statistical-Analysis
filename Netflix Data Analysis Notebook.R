...
## Loading the dataset
setwd('/Users/tarekatassi/Library/CloudStorage/OneDrive-Personal/Courses/Netflix Data CaseStudy')
...
n2_graph <- ggplot(data=n2)+geom_col(mapping=aes(x=Date, y=total, fill=ifelse(total==max(total),"red","grey")))+labs(title="Netflix Movies released by date of each month")+theme_minimal()+scale_fill_manual(values = c("#2d2d2d","#E50914"))+theme(legend.position="none",plot.title=element_text(family="Bebas Neue",size=30,color="#E50914"), axis.title.x=element_blank(),axis.title.y=element_blank(),panel.grid.major.x=element_blank(),panel.grid.minor = element_blank(),text=element_text(size=20))
